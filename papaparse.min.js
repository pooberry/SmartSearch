...
! function (e, t) {
	...
	var s, e, f = "undefined" != typeof self ? self : "undefined" != typeof window ? window : void 0 !== f ? f : {},
		...
		k = {
			parse: function (e, t) {
				...
				if (t.dynamicTyping = i, t.transform = !!M(t.transform) && t.transform, t.worker && k.WORKERS_SUPPORTED) {
					var n = function () {
						...
						return t.onmessage = v, t.id = u++, h[t.id] = t
					}();
				...
				var r = null; {
					if (e === k.NODE_STREAM_INPUT) return (r = new g(t)).getStream();
			...
			unparse: function (e, t) {
				...
				function a(e, t) {
					...
					if (n && f) {
						for (var s = 0; s < e.length; s++) 0 < s && (i += d), i += l(e[s], s);
						0 < t.length && (i += c)
					}
				...
				function l(e, t) {
					...
					var i = "boolean" == typeof n && n || n instanceof Array && n[t] || function (e, t) {
						...
						return !1
					}(e, k.BAD_DELIMITERS) || -1 < e.indexOf(d) || " " === e.charAt(0) || " " === e.charAt(e.length - 1);
					return i ? r + e + r : e
				}
			}
		};
		...
		d.fn.parse = function (o) {
			...
			function e() {
				if (0 !== h.length) {
					...
					}, k.parse(r.file, r.instanceConfig)
	...
	function l(e) {
		var n;
		(e = e || {}).chunkSize || (e.chunkSize = k.RemoteChunkSize), c.call(this, e), this._nextChunk = r ? function () {
			...
			if (this._finished) this._chunkLoaded();
				...
				if (n = new XMLHttpRequest, this._config.withCredentials && (n.withCredentials = this._config.withCredentials), r || (n.onload = R(this._chunkLoaded, this), n.onerror = R(this._chunkError, this)), n.open("GET", this._input, !r), this._config.downloadRequestHeaders) {
	...
	function p(e) {
		var n, r;
		(e = e || {}).chunkSize || (e.chunkSize = k.LocalChunkSize), c.call(this, e);
		var s = "undefined" != typeof FileReader;
		this.stream = function (e) {
			this._input = e, r = e.slice || e.webkitSlice || e.mozSlice, s ? ((n = new FileReader).onload = R(this._chunkLoaded, this), n.onerror = R(this._chunkError, this)) : n = new FileReaderSync, this._nextChunk()
	...
	function m(e) {
		...
		}, this._streamData = R(function (e) {
			try {
				t.push("string" == typeof e ? e : e.toString(this._config.encoding)), i && (i = !1, this._checkIsFinished(), this.parseChunk(t.shift()))
			} catch (e) {
				this._streamError(e)
			}
		}, this), this._streamError = R(function (e) {
	...
	function g(e) {
		...
		this._onCsvData = function (e) {
			for (var t = e.data, i = 0; i < t.length; i++) a.push(t[i]) || this._handle.paused() || this._handle.pause()
		}, this._onCsvComplete = function () {
			a.push(null)
		}, i.step = R(this._onCsvData, this), i.complete = R(this._onCsvComplete, this), c.call(this, i), this._nextChunk = function () {
			...
			decodeStrings: !1,
			read: R(this._onRead, this),
			write: R(this._onWrite, this)
		})).once("finish", R(this._onWriteComplete, this))
	}

	function i(m) {
		var a, o, h, n = /^\s*-?(\d*\.?\d+|\d+\.?\d*)(e[-+]?\d+)?\s*$/i,
		...
		if (M(m.step)) {
			...
			m.step = function (e) {
				if (d = e, p()) l();
					...
					i += e.data.length, m.preview && i > m.preview ? o.abort() : c(d, t)
				}
			}
		}
		...
		function l() {
			if (d && h && (v("Delimiter", "UndetectableDelimiter", "Unable to auto-detect delimiting character; defaulted to '" + k.DefaultDelimiter + "'"), h = !1), m.skipEmptyLines)
				for (var e = 0; e < d.data.length; e++) g(d.data[e]) && d.data.splice(e--, 1);
			return p() && function () {
					if (!d) return;
				...
				function () {
					...
					for (var e = 0; e < d.data.length; e++) {
						...
						d.data[e] = i, m.header && (t > f.length ? v("FieldMismatch", "TooManyFields", "Too many fields: expected " + f.length + " fields but parsed " + t, s + e) : t < f.length && v("FieldMismatch", "TooFewFields", "Too few fields: expected " + f.length + " fields but parsed " + t, s + e))
					}
		...
		this.parse = function (e, t, i) {
			...
			if (m.newline || (m.newline = function (e, t) {
					...
					return a >= n.length / 2 ? "\r\n" : "\r"
				}(e, n)), h = !1, m.delimiter) M(m.delimiter) && (m.delimiter = m.delimiter(e), d.meta.delimiter = m.delimiter);
			else {
				var r = function (e, t, i, n) {
					for (var r, s, a, o = [",", "\t", "|", ";", k.RECORD_SEP, k.UNIT_SEP], h = 0; h < o.length; h++) {
						var u = o[h],
							f = 0,
							d = 0,
							c = 0;
				...
				r.successful ? m.delimiter = r.bestDelimiter : (h = !0, m.delimiter = k.DefaultDelimiter), d.meta.delimiter = m.delimiter
			}
			var s = E(m);
			return m.preview && m.header && s.preview++, a = e, o = new C(s), d = o.parse(a, t, i), l(), u ? {
				meta: {
					paused: !0
				}
			} : d || {
			...
			e = !0, o.abort(), d.meta.aborted = !0, M(m.complete) && m.complete(d), a = ""
		}
	}
	...
	function C(e) {
		...
		if (void 0 !== e.escapeChar && (L = e.escapeChar), ("string" != typeof x || -1 < k.BAD_DELIMITERS.indexOf(x)) && (x = ","), O === x) throw "Comment character same as delimiter";
		!0 === O ? O = "#" : ("string" != typeof O || -1 < k.BAD_DELIMITERS.indexOf(O)) && (O = !1), "\n" !== T && "\r" !== T && "\r\n" !== T && (T = "\n");
		var A = 0,
			F = !1;
		this.parse = function (n, t, i) {
			...
			if (P || !1 !== P && -1 === n.indexOf(S)) {
				for (var c = n.split(T), l = 0; l < c.length; l++) {
					if (f = c[l], A += f.length, l !== c.length - 1) A += T.length;
					else if (i) return E();
					if (!O || f.substr(0, a) !== O) {
						if (o) {
			...
			for (var p, _ = n.indexOf(x, A), m = n.indexOf(T, A), g = new RegExp(L.replace(/[-[\]/{}()*+?.\\^$|]/g, "\\$&") + S, "g");;)
				if (n[A] !== S)
					if (O && 0 === f.length && n.substr(A, a) === O) {
						if (-1 === m) return E();
						A = m + s, m = n.indexOf(T, A), _ = n.indexOf(x, A)
					} else if (-1 !== _ && (_ < m || -1 === m)) f.push(n.substring(A, _)), A = _ + e, _ = n.indexOf(x, A);
			else {
				if (-1 === m) break;
				if (f.push(n.substring(A, m)), w(m + s), o && (R(), F)) return E();
				...
				for (p = A, A++;;) {
					...
					if (p === r - 1) return b(n.substring(A, p).replace(g, S));
					if (S !== L || n[p + 1] !== L) {
						if (S === L || 0 === p || n[p - 1] !== L) {
							var v = C(-1 === m ? _ : Math.min(_, m));
							if (n[p + 1 + v] === x) {
								f.push(n.substring(A, p).replace(g, S)), A = p + 1 + v + e, _ = n.indexOf(x, A), m = n.indexOf(T, A);
								break
							}
							var k = C(m);
							if (n.substr(p + 1 + k, s) === T) {
								if (f.push(n.substring(A, p).replace(g, S)), w(p + 1 + k + s), _ = n.indexOf(x, A), o && (R(), F)) return E();
								if (D && h.length >= D) return E(!0);
								break
							}
							u.push({
			...
			function b(e) {
				return i || (void 0 === e && (e = n.substr(A)), f.push(e), A = r, y(f), o && R()), E()
			}

			function w(e) {
				A = e, y(f), f = [], m = n.indexOf(T, A)
			}
	...
	return o ? f.onmessage = function (e) {
		...
		a = !0
	}, !0) : a = !0), (l.prototype = Object.create(c.prototype)).constructor = l, (p.prototype = Object.create(c.prototype)).constructor = p, (_.prototype = Object.create(_.prototype)).constructor = _, (m.prototype = Object.create(c.prototype)).constructor = m, (g.prototype = Object.create(c.prototype)).constructor = g, k
});